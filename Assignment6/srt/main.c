...
#include <math.h>
#include <pthread.h> 
//Included to ensure it works with
...
// Should be a global variables
int nthreads; //number of pthreads passed in
scene_t scene; //Used to set the scene using the sphereflake_recursion method
float data[width][height][3]; //Create a global float array to store value from each thread

//Function to be called by each pthread (taken from main) 
void *multithreadFunc(void *threadNum){

    //Start and end condition for the for loop that goes through each pixel.
    int start = (*(int*)threadNum) * (width/nthreads);
    int end = (*(int*)threadNum) == (nthreads-1) ? width : (*(int*)threadNum + 1) * (width/nthreads);
    ...
    for( int px = start; px < end; px++ )
    {
        ...
        for( int py = 0; py < height; ++py )
        {
            ...
            //Use data array to store values
            data[px][py][0] = gamma( pixel_color[0] ) * max_color;
            data[px][py][1] = gamma( pixel_color[1] ) * max_color;
            data[px][py][2] = gamma( pixel_color[2] ) * max_color;

            /* enforce caps, replace with real gamma */
            for( int i=0; i<3; i++)
                data[px][py][i] = max( min(data[px][py][i], 255), 0);
        }
    }
    //Return null since method's return type is a pointer
    return NULL;
}
...
{
    //Set nthreads to first argument if 2 arguments were passed in
    //Convert the string to an integer so that it suits the int data type
    nthreads = argc == 2 ? atoi( argv[1] ) : 0;

    //For multithreading, must have more than 1 thread
    if( nthreads < 1 )
    {
      //Print to stderr and return with value 1
    ...
    //Create array of pthread objects of size of nthreads argument passed in
    pthread_t threads[nthreads];
    //Temp int to create threads 
    int tcreate;
    /*Allocate enough space to store threads ids to be passed in as
    an argument when calling the pthread_create method
    */
    int *tid = (int*)malloc(sizeof(int)*nthreads);
    
    //Create threads using the pthread_create method
    //If threads fail to create, print to stderr and exit with tcreate as the exit code
    for (int i=0; i<nthreads; i++){
        //Set tid to the current value of the loop
        tid[i] = i;
        //Call the pthread_create method
        tcreate = pthread_create(&threads[i], NULL, multithreadFunc, (void*)&tid[i]);
        if (tcreate){
    ...
    //Join threads using the pthread_join method
    //If threads fail to create, print to stderr and exit with tcreate as the exit code
    for (int i=0; i<nthreads; i++){
        tcreate = pthread_join(threads[i], NULL);
        if (tcreate){
    ...
    */
    for (int i=0; i<width; i++){
        ...
        //Print new line after going through inner for loop
        printf("\n");
    }

    //Free Scene
    free_scene( &scene );
    //Free space used to create tids
    free(tid);
